...
// int code_directory_verify_special_slots(MachOSlice *slice, CS_CodeDirectory *codeDirectory, uint8_t *hashes) {
...
int code_directory_verify_code_slots(MachOSlice *slice, CS_CodeDirectory *codeDirectory, uint8_t *hashes) {
    ...
    for (int i = 0; i < codeDirectory->nCodeSlots; i++) {
        ...
        macho_slice_read_at_offset(slice, dataOffsetToRead, dataSizeToRead, data);
...
int macho_slice_parse_code_directory_blob(MachOSlice *slice, uint32_t codeDirectoryOffset, CS_CodeDirectory *codeDirectoryOut, bool printSlots, bool verifySlots)
{
