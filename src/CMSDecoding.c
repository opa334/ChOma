...
int cms_data_decode(uint8_t *cmsDERData, size_t cmsDERLength) {
    ...
    // Check against the first bytes being 0x30 0x80 - this is not valid DER
    if (cmsDERData[0] == 0x30 && cmsDERData[1] == 0x80) {
        printf("Error: DER data is invalid - first two bytes are 0x30 0x80. Please sign using ldid instead of codesign.\n");
        return -1;
    }
